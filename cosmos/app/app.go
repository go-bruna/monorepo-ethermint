...
func NewChainApp(
	...
	baseAppOptions = append(baseAppOptions, func(app *baseapp.BaseApp) {
		mempool := mempool.NewPriorityMempool(mempool.PriorityNonceMempoolConfig[int64]{
			...
			SignerExtractor: ethermintapp.NewEthSignerExtractionAdapter(mempool.NewDefaultSignerExtractionAdapter()),
		})
	...
	genesisModuleOrder := []string{
		...
		minttypes.ModuleName, evidencetypes.ModuleName, authz.ModuleName,
	...
	anteHandler, err := ethermintante.NewAnteHandler(ethermintante.HandlerOptions{
	...
	// Optional SetPostHandler

	// must be before Loading version
	// requires the snapshot store to be created and registered as a BaseAppOption
	// see cmd/incod/root.go: 206 - 214 approx
