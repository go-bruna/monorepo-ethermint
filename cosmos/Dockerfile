...
# then log output of file /code/bin/incod
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/incod \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/incod | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/incod /usr/bin/incod
